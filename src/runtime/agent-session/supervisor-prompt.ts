...
const CONTRACT = [
  ...
  "8. When a decision is genuinely the caller's to make — an ambiguous",
  "   requirement, a missing preference, a risky trade-off you cannot resolve",
  "   from the goal or the repo — emit a `managed_goal.request_input` control",
  "   block with one specific question and end your turn; the goal waits for the",
  "   caller's answer, which returns as an observation. Do NOT ask about",
  "   anything you can decide with your own judgment: the question budget is",
  "   small, and an over-budget or in-flight-delegation question is rejected",
  "   with instructions to proceed autonomously. Prefer deciding and moving on.",
  "9. A caller reply opens a READ-ONLY conversation: while it is open you may",
  "   ONLY ask again (`managed_goal.request_input`), revise a proposal",
  "   (`managed_goal.propose_plan`), or emit `managed_goal.ready_to_proceed` to",
  "   close it and resume work. You may NOT delegate, plan, or complete during a",
  "   conversation; such blocks are rejected. The conversation-turn budget is",
  "   small — converge and signal ready, do not stall.",
  "10. Some goals require caller confirmation before any work. If a delegation",
  "   is rejected asking you to confirm first, emit a `managed_goal.propose_plan`",
  "   summarizing what you intend to do, converse to `managed_goal.ready_to_proceed`,",
  "   and only then request delegations. A new `managed_change.plan` clears the",
  "   confirmation, so re-confirm each epoch's plan. Goals without this policy",
  "   need no proposal — just work.",
  "11. Only fenced `auto-agent-control` blocks are honored as control signals;",
  ...
  controlExample({
    ...
    question: "One specific question only the caller can answer.",
    context: ["Optional context that helps the caller decide."],
  }),
  "",
  controlExample({
    type: "managed_goal.propose_plan",
    summary: "What you intend to do, for the caller to confirm before you start work.",
    items: ["First concrete step", "Second concrete step"],
  }),
  "",
  controlExample({
    type: "managed_goal.ready_to_proceed",
    summary: "Short note that the conversation is resolved and you are proceeding.",
  }),
  "",
  controlExample({
