...
- [ ] 1.1 Add provider settings domain/config types for mock and Codex Local selections, including sanitized status fields.
...
- [ ] 2.1 Implement Codex CLI command detection using saved manual path, PATH lookup, and common local install locations where practical.
...
- [ ] 4.1 Refactor backend runtime selection so goal start uses current saved provider settings instead of only startup environment configuration.
...
- [ ] 5.1 Add dashboard API client functions and types for provider settings, detection, and connection test endpoints.
