...
- [ ] 1.1 Add explicit scope assessment and binary scope vote domain types.
...
- [ ] 3.1 Replace completion-gate quorum semantics with binary scope vote semantics.
...
- [ ] 4.1 Remove post-implementation voting from `IMPLEMENT_DIRECTLY` and close the current work item after marking the step completed.
- [ ] 4.2 Add separate `maxScopeAssessmentAttempts` and `maxScopeRefinementRounds` loop bounds.
...
- [ ] 5.1 Update mock runtime defaults and deterministic planner/voter behavior for scope refinement.
...
- [ ] 6.1 Run focused domain, voter, planner, runtime, mock runtime, and backend API tests.
