# Agent Live Status Specification

## Purpose

Define the compact, durable, pipeline-aware status projected for a goal so the
dashboard can show current authoritative activity without interpreting raw
events or provider processes.
